...
class PuppeteerHar {
     ...
     * @return {Promise<void>}
     */
    async start({ path, saveResponse, captureMimeTypes, includeResourcesFromDiskCache } = {}) {
        this.inProgress = true;
        this.saveResponse = saveResponse || false;
        this.captureMimeTypes = captureMimeTypes || ['text/html', 'application/json'];
        this.includeResourcesFromDiskCache = includeResourcesFromDiskCache || false;
        this.path = path;
    ...
    async stop() {
        ...
        const har = harFromMessages(
            this.page_events.concat(this.network_events),
            {
              ...
              includeResourcesFromDiskCache: this.includeResourcesFromDiskCache
            }
        );
        this.cleanUp();
        if (this.path) {
